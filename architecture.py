...
class Unet3D(nn.Module):
    def __init__(
        ...
        self.time_rel_pos_bias = RelativePositionBias(heads = attn_heads, max_distance = 32) # realistically will not be able to generate that many frames of fields... yet
...
def fields_tensor_to_gif(tensor, path, duration = 120, loop = 0, optimize = True):
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        print(f'found {len(self.ds)} fieldss as gif files at {folder}')
        assert len(self.ds) > 0, 'need to have at least 1 fields to start training (although 1 is not great, try 100k)'
    ...
    def train(
        ...
        while self.step < self.train_num_steps:
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                all_fieldss_list = list(map(lambda n: self.ema_model.sample(batch_size=n), batches))
                all_fieldss_list = torch.cat(all_fieldss_list, dim = 0)

                all_fieldss_list = F.pad(all_fieldss_list, (2, 2, 2, 2))

                one_gif = rearrange(all_fieldss_list, '(i j) c f h w -> c f (i h) (j w)', i = self.num_sample_rows)
                fields_path = str(self.results_folder / str(f'{milestone}.gif'))
                fields_tensor_to_gif(one_gif, fields_path)
                log = {**log, 'sample': fields_path}
                self.save(milestone)
